...
/*   Updated: 2019/04/28 21:17:52 by gleonett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
short dfs(t_tbhash *i, short lvl, short len)
{
	...
	if (num_links == 0 || len > g_num_rooms || i->flag == 1)
		return (-1);
...
//		ft_printf("\n");
	...
	else
	{
		...
		while (num_links > 0 && i->num_links > 0)
		{
			...
			if (i->links[j] != NULL)
			{
				if (i->links[j]->flag != 1 && i->links[j]->p_x >= i->p_x &&
...
short			**prep_dfs(t_tbhash **th, t_mtrx *mtrx, int num_a_r[2])
{
	...
	CH_NULL(g_ways = (short **)malloc(sizeof(short *) * (num_a_r[1] * 6 +
			1)));
	i = -1;
